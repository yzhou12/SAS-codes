...
data look;
              set iga;
              where PARAMCD='PGA' and avisitn>0;
              keep SUBJID AVISIT AVISITN TRTA TRTAN AVAL BASE CRIT1FN dtype ANL01FL;
run;

data look1; set look1;
	chg = aval - base;
	iga = (aval<2 and chg <=-2) and chg ne '.';
run;
...
title "IGA response at Week 6, Chan and Zhang 1999, mITT (NRI)";
